...
async def lifespan(app: FastAPI):
    # Startup: register running asyncio loop and start background processing thread
    state.loop = asyncio.get_running_loop()
    thread = threading.Thread(target=stream_processing_thread, daemon=True)
    thread.start()
    yield
    # Clean shutdown handling if needed
...
METRIC_DEFECTS = Counter(
    f"{PROMETHEUS_PREFIX}defects_detected_total",
    "Total number of defects detected by class",
    ["defect_type"],
)
...
METRIC_CAMERA_ONLINE = Gauge(
    f"{PROMETHEUS_PREFIX}camera_online",
    "Operational status of the camera stream (1 for online, 0 for offline)",
)
METRIC_ACTIVE_WS = Gauge(
    f"{PROMETHEUS_PREFIX}active_websocket_connections",
    "Number of active client WebSocket connections",
)
...
def simulate_plc_broadcast(detections: List[Dict[str, Any]]):
    ...
    broadcast_ws_message({"type": "plc_broadcast", "payload": plc_payload})
...
def stream_processing_thread():
    ...
    """
    processor = VideoStreamProcessor(source=state.source_path, model_path="best_industrial_defect.onnx")
        ...
        while True:
            ...
            if state.source_changed:
                ...
                processor = VideoStreamProcessor(
                    source=state.source_path, model_path="best_industrial_defect.onnx"
                )
            ...
            # Update Prometheus
            METRIC_FRAMES.inc()
            METRIC_LATENCY.observe(metrics["inference_ms"] / 1000.0)
            for det in detections:
...
def get_classes():
    """
    Returns metadata for all detectable defect classes including labels, colors, and thresholds.
    """
    return {
        "classes": detector.get_class_metadata() if detector else []
    }
...
def export_report():
    """
    Generates a structured system telemetry and defect summary report for quality audits.
    """
    with state.lock:
        uptime = time.time() - state.start_time if state.camera_online else 0.0
        return {
            "service": "Industrial Defect Detection System",
            "timestamp": time.time(),
            "uptime_seconds": int(uptime),
            "active_source": str(state.source_path),
            "camera_online": state.camera_online,
            "device": detector.device_name if detector else "unknown",
            "defect_classes": detector.classes if detector else [],
            "latest_detections": state.latest_detections,
            "latest_metrics": state.metrics,
            "total_active_clients": len(state.active_websockets),
        }
    ...
    if file_extension not in allowed_extensions:
        raise HTTPException(
            ...
            detail=(
                ...
                "Allowed types: JPG, JPEG, PNG, BMP, WEBP, MP4, AVI, MOV, MKV, WEBM."
            ),
        )
